...
class ConjugationNotes():
    ...
    def irregular_nature(self):
        if len(self._conjugation_note_list) > 0:
            return reduce(lambda x, y: x if x > y else y,
                          [conjugation_note.irregular_nature for conjugation_note in self._conjugation_note_list])
        else:
    ...
    def conjugation(self):
        ...
        if explicit_conjugation_ is not None:
            ...
            return (self.core_verb if self.core_verb is not None else '') + (
                self.ending if self.ending is not None else '')
    ...
    def __raise(self, msg, traceback_=None):
        msg_ = "{0}: (tense={1},person={2}): {3}".format(self.phrase.full_phrase,
                                                         Tense[
                                                             self.tense].human_readable if self.tense is not None else "-",
                                                         Person[
                                                             self.person].human_readable if self.person is not None else "-",
                                                         msg)
        cs_error(">>>>>>", msg_)
    ...
    def __repr__(self):
        return {
            'tense': self.tense,
            'person': self.person,
            'changes': [conjugation_note.__repr__() for conjugation_note in self._conjugation_note_list if
                        conjugation_note.is_irregular]
        }.__str__()
